...
class NotificationClient:
    ...
    def bounty_issued(self, bounty_id, **kwargs):
        ...
        create_dashboard_notification(bounty_id, BOUNTY_ISSUED, bounty.user, bounty.bounty_created, string_data)
    ...
    def bounty_fulfilled(self, bounty_id, fulfillment_id, **kwargs):
        ...
        # to fulfiller
        create_dashboard_notification(bounty_id, FULFILLMENT_SUBMITTED, fulfillment.user, fulfillment.fulfillment_created, string_data_fulfiller, is_activity=False)
        # to bounty issuer
        create_dashboard_notification(bounty_id, FULFILLMENT_SUBMITTED, bounty.user, fulfillment.fulfillment_created, string_data_issuer)
    ...
    def bounty_activated(self, bounty_id, event_date, **kwargs):
        ...
        create_dashboard_notification(bounty_id, BOUNTY_ACTIVATED, bounty.user, event_date, string_data)


    def bounty_issued_and_activated(self, bounty_id, event_date, **kwargs):
        bounty = Bounty.objects.get(id=bounty_id)
        string_data = BOUNTY_ACTIVATED_STR.format(bounty_title=bounty.title)
        create_dashboard_notification(bounty_id, BOUNTY_ISSUED_ACTIVATED, bounty.user, event_date, string_data)
    ...
    def fulfillment_accepted(self, bounty_id, fulfillment_id, **kwargs):
        ...
        create_dashboard_notification(bounty_id, FULFILLMENT_ACCEPTED, bounty.user, fulfillment.accepted_date, string_data_issuer)
        create_dashboard_notification(bounty_id, FULFILLMENT_ACCEPTED, fulfillment.user, fulfillment.accepted_date, string_data_fulfiller, is_activity=False)
    ...
    def fulfillment_updated(self, bounty_id, fulfillment_id, event_date, **kwargs):
        ...
        create_dashboard_notification(bounty_id, FULFILLMENT_UPDATED, bounty.user, event_date, string_data_issuer, is_activity=False)
        create_dashboard_notification(bounty_id, FULFILLMENT_UPDATED, fulfillment.user, event_date, string_data_fulfiller, is_activity=False)


    def bounty_killed(self, bounty_id, event_date, **kwargs):
        bounty = Bounty.objects.get(id=bounty_id)
        string_data = BOUNTY_KILLED_STR.format(bounty_title=bounty.title)
        create_dashboard_notification(bounty_id, BOUNTY_KILLED, bounty.user, event_date, string_data)
    ...
    def contribution_added(self, bounty_id, event_date, **kwargs):
        ...
        create_dashboard_notification(bounty_id, CONTRIBUTION_ADDED, bounty.user, event_date, string_data)


    def deadline_extended(self, bounty_id, event_date, **kwargs):
        bounty = Bounty.objects.get(id=bounty_id)
        string_data = DEADLINE_EXTENDED_STR.format(bounty_title=bounty.title)
        create_dashboard_notification(bounty_id, DEADLINE_EXTENDED, bounty.user, event_date, string_data)


    def bounty_changed(self, bounty_id, event_date, **kwargs):
        bounty = Bounty.objects.get(id=bounty_id)
        string_data = BOUNTY_CHANGED_STR.format(bounty_title=bounty.title)
        create_dashboard_notification(bounty_id, BOUNTY_CHANGED, bounty.user, event_date, string_data)
    ...
    def issuer_transferred(self, bounty_id, transaction_from, inputs, event_date, **kwargs):
        ...
        create_dashboard_notification(bounty_id, ISSUER_TRANSFERRED, original_user, event_date, string_data_transferrer)
        create_dashboard_notification(bounty_id, TRANSFER_RECIPIENT, bounty.user, event_date, string_data_recipient)


    def payout_increased(self, bounty_id, event_date, **kwargs):
        bounty = Bounty.objects.get(id=bounty_id)
        string_data = PAYOUT_INCREASED_STR.format(bounty_title=bounty.title)
        create_dashboard_notification(bounty_id, PAYOUT_INCREASED, bounty.user, event_date, string_data)


    def bounty_expired(self, bounty_id, event_date, **kwargs):
        bounty = Bounty.objects.get(id=bounty_id)
        string_data = BOUNTY_EXPIRED_STR.format(bounty_title=bounty.title)
        create_dashboard_notification(bounty_id, BOUNTY_EXPIRED, bounty.user, event_date, string_data)
