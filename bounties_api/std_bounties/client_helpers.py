...
bounty_data_keys = ['description', 'title', 'sourceFileName', 'sourceFileHash', 'sourceDirectoryHash', 'webReferenceUrl']
fulfillment_data_keys = ['description', 'sourceFileName', 'sourceFileHash', 'sourceDirectoryHash']
...
def map_fulfillment_data(data_hash, bounty_id, fulfillment_id):
    ipfs_hash = data_hash
    if len(ipfs_hash) != 46 or not ipfs_hash.startswith('Qm'):
        logger.error('Data Hash Incorrect for fulfillment on bounty: {:d} fulfillment: {:d}'.format(bounty_id, fulfillment_id))
        data_JSON = "{}"
    else:
        data_JSON = ipfs.cat(ipfs_hash)
    if len(ipfs_hash) == 0:
...
def get_token_pricing(token_symbol, token_decimals, value):
    try:
        ...
        usd_price = ((Decimal(value) / Decimal(pow(10, token_decimals))) * Decimal(token_model.price_usd)).quantize(Decimal(10) ** -8)
...
def map_token_data(pays_tokens, token_contract, amount):
    ...
    usd_price, token_model = get_token_pricing(token_symbol, token_decimals, amount)
