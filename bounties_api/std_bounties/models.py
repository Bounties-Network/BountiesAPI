...
class Bounty(BountyAbstract):
    ...
    def save(self, *args, **kwargs):
        ...
        if not created and not user.profile_hash:
            user.name = self.issuer_name if self.issuer_name else user.name
            user.email = self.issuer_email if self.issuer_email else user.email
            user.save()
        self.user = user
...
class Fulfillment(models.Model):
    ...
    fulfiller_profile_image = models.CharField(max_length=256, blank=True)
    description = models.TextField(blank=True)
    sourceFileName = models.CharField(max_length=256, blank=True)
    sourceFileHash = models.CharField(max_length=256, blank=True)
    sourceDirectoryHash = models.CharField(max_length=256, blank=True)
