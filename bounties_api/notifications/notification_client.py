...
class NotificationClient:
    ...
    def bounty_activated(self, bounty_id, event_date, **kwargs):
        ...
        create_notification(bounty, BOUNTY_ACTIVATED, bounty.user, event_date, string_data, 'Bounty Activated')


    def bounty_issued_and_activated(self, bounty_id, event_date, **kwargs):
        bounty = Bounty.objects.get(id=bounty_id)
        string_data = BOUNTY_ACTIVATED_STR.format(bounty_title=bounty.title)
        create_notification(bounty, BOUNTY_ISSUED_ACTIVATED, bounty.user, event_date, string_data, 'Bounty Issued and Activated')
    ...
    def fulfillment_accepted(self, bounty_id, fulfillment_id, **kwargs):
        ...
        create_notification(bounty, FULFILLMENT_ACCEPTED, fulfillment.user, fulfillment.accepted_date, string_data_fulfiller, 'Your Submission was Accepted', is_activity=False)
    ...
    def bounty_expired(self, bounty_id, event_date, **kwargs):
        ...
        create_notification(bounty, BOUNTY_EXPIRED, bounty.user, event_date, string_data, 'Bounty Expired')
