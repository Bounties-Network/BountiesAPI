...
def get_base_bounty_values(bounty):
    ...
    base_fields['fulfillmentAmount'] = calculate_token_quantity(bounty.fulfillmentAmount, bounty.decimals)
    ...
    base_fields['token_lock_price'] = 'Unkown Price' if not bounty.token_lock_price else Decimal(bounty.token.token_lock_price).quantize(Decimal(10) ** -2)
