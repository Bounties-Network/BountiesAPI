...
DEFAULT_PLATFORM_QUERY = Q(bounty__platform=DEFAULT_PLATFORM) | Q(bounty__platform__isnull=True) | Q(bounty__platform="")
...
def generate_timeline(time_frame, platform=DEFAULT_PLATFORM): 
    ...
    if platform == DEFAULT_PLATFORM: 
        ...
        bounty_state_platform = bounty_state_platform.select_related('bounty').filter(bounty__platform=platform)
        fulfillment_platform = fulfillment_platform.select_related('bounty').filter(bounty__platform=platform)

    bounties_state_frame_day = bounty_state_platform.filter(change_date__range=time_frame, bountyStage=DRAFT_STAGE)
    bounties_state_frame = bounty_state_platform.filter(change_date__lte=time_frame[1])
    fulfillment_accepted_frame = fulfillment_platform.filter(accepted_date__lte=time_frame[1])
    fulfillment_accepted_frame_day = fulfillment_platform.filter(accepted_date__range=time_frame)
    fulfillment_submitted_frame = fulfillment_platform.filter(fulfillment_created__lte=time_frame[1])
    fulfillment_submitted_frame_day = fulfillment_platform.filter(fulfillment_created__range=time_frame)
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        platforms_query = BountyState.objects.distinct('bounty__platform')
        platforms = [platform.bounty.platform for platform in platforms_query if platform.bounty.platform] + [ALL_PLATFORM]

        for platform in platforms:
            needs_genesis = not BountiesTimeline.objects.filter(platform=platform, is_week=False).exists()
            ...
            if needs_genesis:
                ...
                for day in bounties_by_day:
                    bounty_day = generate_timeline(day_bounds(day), platform=platform)

                    bounty_day.save()
            else:
                last_update = BountiesTimeline.objects.filter(platform=platform, is_week=False).order_by('date').last()
                ...
                for day in days:
                    bounty_day = generate_timeline(day_bounds(day), platform=platform)
                    bounty_points = BountiesTimeline.objects.filter(date=day.date(), platform=platform, is_week=False)
        ...
        for platform in platforms:
            needs_genesis = not BountiesTimeline.objects.filter(platform=platform, is_week=True).exists()
            ...
            if needs_genesis:
                ...
                for week in bounties_by_week:
                    bounty_week = generate_timeline(week_bounds(week), platform=platform)
                    bounty_week.is_week = True

                    bounty_week.save()
            else:
                last_update = BountiesTimeline.objects.filter(platform=platform, is_week=True).order_by('date').last()
                since = arrow.get(last_update.date).to('utc')
                ...
                for week in weeks:
                    ...
                    bounty_points = BountiesTimeline.objects.filter(date=bounds[0].date(), platform=platform, is_week=True)
